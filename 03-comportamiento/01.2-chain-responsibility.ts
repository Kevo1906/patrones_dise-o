...
class Supervisor extends BaseApprover {
  ...
  override approveRequest(amount: number): void {
    if(amount<=1000){
      console.log(`Monto: ${amount}$ Aprobado por el Supervisor`)
      return
    }
    console.log('Supervisor no puede aprobar esa cantidad pasando al siguiente')
    super.next(amount)
...
class Manager extends BaseApprover {
  ...
  override approveRequest(amount: number): void {
    if(amount<=5000){
      console.log(`Monto: ${amount}$ Aprobado por el Manager`)
      return
    }
    console.log('Manager no puede aprobar esa cantidad pasando al siguiente')
    super.next(amount)
...
class Director extends BaseApprover {
  override approveRequest(amount: number): void {
    console.log(`Monto: ${amount}$ Aprobado por el Director`)
  }
